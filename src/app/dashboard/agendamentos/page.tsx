...
import { Plus, Calendar, ChevronDown, ChevronUp, Trash2, CheckCircle, XCircle, Pencil, X } from 'lucide-react'
...
export default function AgendamentosPage() {
  ...
  function openEdit(appt: Appointment) {
    const localDT = new Date(appt.scheduledAt).toISOString().slice(0, 16)
    setEditForm({
      title: appt.title,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gerencie seus encontros e acompanhe os lembretes automáticos</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="font-semibold text-gray-900">Editar agendamento</h2>
        ...
        <div key={appt.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{appt.customerPhone}</p>
            </div>
          ...
          {expanded === appt.id && (
            <div className="border-t border-gray-100 px-5 py-4 bg-gray-50">
              {appt.notes && (
                <p className="text-sm text-gray-600 mb-3"><span className="font-medium">Obs:</span> {appt.notes}</p>
              )}
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Lembretes automáticos</p>
              <div className="grid grid-cols-2 gap-2">
                {appt.notifications.length === 0 ? (
                  <p className="text-sm text-gray-400 col-span-2">Nenhum lembrete agendado</p>
                ) : (
                  appt.notifications.map((n) => (
                    <div key={n.id} className="flex items-center justify-between bg-white rounded-lg border border-gray-200 px-3 py-2">
                      <div>
                        <span className="text-sm font-medium text-gray-700">{NOTIF_LABELS[n.type]}</span>
                        <p className="text-xs text-gray-400">{formatDateTime(n.scheduledAt)}</p>
                      </div>
